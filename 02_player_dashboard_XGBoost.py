...
# 사용자 설정
# ==========================================
# [수정됨] XGBoost 결과 이미지가 저장된 폴더명
IMAGE_FOLDER = 'dashboard_XGBoost_imgs' 

# 재생 속도 (밀리초 단위, 200ms = 0.2초)
PLAY_INTERVAL = 200  
...
class DashboardPlayer:
    def __init__(self, folder_path):
        ...
        if self.num_frames == 0:
            ...
            print("폴더명이 정확한지, 혹은 이미지 생성 코드를 먼저 실행했는지 확인해주세요.")
            return
        ...
        self.anim.event_source.stop() # 시작 시 정지 상태

        print(f"!! 플레이어 실행됨: '{self.folder_path}' 폴더에서 총 {self.num_frames}장의 이미지를 로드했습니다.")
        plt.show()
    ...
    def load_image_paths(self):
        ...
        # 정렬: 파일명에 포함된 숫자를 기준으로 정렬 (예: stage_2가 stage_10보다 앞에 오도록)
    ...
    def update_title(self):
        ...
        self.ax.set_title(f"Dashboard Viewer - XGBoost {filename} ({self.current_frame + 1}/{self.num_frames})", fontsize=14, fontweight='bold')

    def on_slider_change(self, val):
        """슬라이더를 움직였을 때 호출"""
        # 애니메이션이 재생 중일 때 슬라이더를 건드리면 잠시 멈추거나 튀는 현상을 방지
